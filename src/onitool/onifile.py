...
RECORD_NODE_ADDED2 = 0x03
RECORD_NODE_ADDED_ALL = (RECORD_NODE_ADDED, RECORD_NODE_ADDED_1_0_0_4, RECORD_NODE_ADDED_1_0_0_5)
...
def parseint(a):
    mybytes = a.read(4)
    return struct.unpack("i", mybytes)[0]
...
def writedadded(a, h, hh):
    ...
    # print h,hh
    a.write(struct.pack("=iiiQQQ", hh["nodetype"], ocodec, hh["frames"], hh["mints"], hh["maxts"],
                        hh["seektable"]))
...
def addprop(a, nid, name, type, value, datalen=0):
    if type == RECORD_INT_PROPERTY:
        ...
        if datalen == 8:
            ...
            c = makestr(name) + struct.pack("=iii", 4 + 4, value,
                                            0)  # last 0 is dummy for leaving space?
...
def copyblock(a, h, b, frame=None, timestamp=None):
    a.seek(h["poffset"], 0)
    hout = dict(rt=h["rt"], nid=h["nid"], fs=h["fs"], ps=h["ps"], undopos=h["undopos"], poffset=0,
                hoffset=b.tell(), nextheader=0)
    writehead(b, hout)
    if h["fs"] > HEADER_SIZE:
        # adjust time
...
def writehead(a, h):
    a.write(struct.pack("5i", MAGIC, h["rt"], h["nid"], h["fs"], h["ps"]) + struct.pack("Q", h[
        "undopos"]))


def readrechead(a, version):
    ...
    magic, rt, nid, fs, ps, undopos = struct.unpack(struct_fmt, h1)
    if magic != MAGIC:
        print("bad magic record", magic)
        return None
    r = dict(rt=rt, nid=nid, fs=fs, ps=ps, poffset=a.tell(), hoffset=p, nextheader=p + ps + fs,
             undopos=undopos, magic=magic)
    return r
...
class Reader:
    ...
    def next(self):
        ...
        h = readrechead(self.file, self.h0["version"])
        # print(h)
        self.lasth = h
        ...
        if h["rt"] == RECORD_NEW_DATA:
            pass
        elif h["rt"] in RECORD_NODE_ADDED_ALL:
...
class Writer:
    ...
    def copyblock(self, header, file):
        ...
        # ANALYZE
        rt = header["rt"]
        if rt in RECORD_NODE_ADDED_ALL:
            hh = dict()
            hh.update(header)
            hh["poffset"] = po
        ...
        elif rt == RECORD_INT_PROPERTY or rt == RECORD_REAL_PROPERTY or rt == \
                RECORD_GENERAL_PROPERTY:
            self.configid += 1
    ...
    def finalize(self):
        ...
        # removed {'rt': 7, 'ps': 0, 'fs': 28, 'poffset': 368218784, 'hoffset': 368218756,
        # 'nid': 2, 'nextheader': 368218784, 'undopos': 1873}
        for q in self.stats.values():
            if not q.emitted:
                # print "writingseektable",q
